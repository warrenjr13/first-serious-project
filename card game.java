...
public class MyClass {
  public static void main(String args[]) {
       ...
       System.out.println("How many cards do each player get?");
         int numCards = scan.nextInt();
       
       Random random_card = new Random();

        int c = random_card.nextInt(52);

        String [] cards_given = new String[numCards * numPlayers];
        
        for(int i = 1; i <= numPlayers; i++){
            System.out.println("Player " + i + ":");
            for(int j = 1; j <= numCards; j++){
                int cardIndex = random_card.nextInt(52);
                System.out.println(All_cards[cardIndex]);
            }
            System.out.println();
        }
